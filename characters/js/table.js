...
angular.module('optc') .run(function($rootScope, $timeout, $storage, MATCHER_IDS) {
    ...
    var addImage = function(data, type, row, meta) {
        return '<table><tr><td style="text-align: center;"><img class="slot small" data-original="' + Utils.getThumbnailUrl(row[0]) + '"></td> ' +
            '<td><a style="text-align:center;width:130px;" ui-sref="main.search.view({ id: ' + parseInt(row[0],10) + '})">' + data + '</a></td></tr></tabla>';
    };
    ...
    var getTableColumns = function() {
        ...
        additionalColumns.forEach(function(x) {
            var title = x
                .replace(/Mínimo cooldown/,'Min CD')
                .replace(/Cooldown inicial/,'Max CD')
    ...
    var data = window.units.filter(function(x) { return x.name; }).map(function(x,n) {
        ...
        additionalColumns.forEach(function(c,n) {
            ...
            else if (c == 'ATK/coste') temp = Math.round(x.maxATK / x.cost * 100) / 100;
            else if (c == 'HP/coste') temp = Math.round(x.maxHP / x.cost * 100) / 100;
            else if (c == 'CMB') temp = x.combo;
            else if (c == 'MAX EXP') temp = x.maxEXP;
            else if (c == 'Mínimo cooldown' || c == 'Cooldown inicial') { 
                var d = cooldowns[x.number];
                if (!d) temp = 'N/A';
                else if (c == 'Mínimo cooldown' && d.constructor == Array) temp = d[1];
                else if (c == 'Cooldown inicial') temp = (d.constructor == Array ? d[0] : d);
                else temp = 'Unknown';
            }
