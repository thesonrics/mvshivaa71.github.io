    ...
    return {
        ...
        template: '<div id="hp"><div id="hpSlider"></div>' + 
            '<div id="hp-rcv"><table><tbody>' +
                ...
                '<tr ng-show="numbers.healPerTurn"><td>{{ numbers.healPerTurn | number }}</td><td>Cura/turno</td></tr>' +
                '<tr title="Nivel de pirata mínimo: {{ numbers.cost.level }}"><td>{{ numbers.cost.cost | number }}</td><td>Coste</td></tr>' +
            '</tbody></table></div>',
        link: function(scope, element, attrs) {
            ...
            scope.setHP = function() {
                var hp = prompt('Introduce el HP:', scope.hp.current), perc = false;
                if (!hp) return;
                hp = hp.trim();
...
directives.turnCounter = function() {
    return {
        ...
        template: '<div id="turns"><div id="turnSlider"></div>' +
            '<div id="turnLabel">{{currentTurns}} {{currentTurns == 1 ? "turno" : "turnos"}} transcurrido/s</div></div>',
        link: function(scope, element, attrs) {
