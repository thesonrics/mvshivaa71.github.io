...
app.controller('ImageGeneratorCtrl', function($scope, $filter, $timeout) {
    ...
    $scope.$watch('numbers',function(numbers) {
        [ 'STR', 'QCK', 'DEX', 'PSY', 'INT' ].forEach(function(which, n) {
            ...
            type(context, { text: 'Daño contra ' + which, style: '10.5px "Open Sans"', x: 515, y: 30 + n * 70, align: 'center' });
        ...
        if ($scope.numbers.cost) {
            ...
            awesome(context, { text: 'f0d6', x: baseX + 8, y: baseY + 50, align: 'center' });
            type(context, { text: cost + ' Coste', x: baseX + 25, y: baseY + 50 });
        }
    },true);
    ...
    type(context, { text: 'Nivel ' + $scope.data.ship[1], x: baseX + 25, y: baseY + 37 });
    baseY += 42;
    ...
    if ($scope.tdata.turnCounter.enabled) {
        ...
        type(context, { text: turns + (turns == 1 ? ' turno' : ' turnos') + ' transcurrido', x: baseX + 25, y: baseY + 20 });
        baseY += 25;
    }

    // specials
    awesome(context, { text: 'f0e7', x: baseX + 8, y: baseY + 20, align: 'center' });
    type(context, { text: 'Especiales activos:', x: baseX + 25, y: baseY + 20 });
    var none = true;
    for (var i=0;i<6;++i) {
    ...
    if (none) {
        type(context, { text: 'Ninguno', x: baseX + 35, y: baseY + 45, maxWidth: 145, truncate: true });
    }
